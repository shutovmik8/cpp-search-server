...
public:
    ...
    template <typename T>
    vector<Document> FindTopDocuments(const string& raw_query, T comp) const {
        const Query query = ParseQuery(raw_query);
        auto matched_documents = FindAllDocuments(query, comp);
...
private:
    ...
    template <typename T>
    vector<Document> FindAllDocuments(const Query& query, T comp) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto &[document_id, term_freq] : word_to_document_freqs_.at(word)) {
                if (comp(document_id, documents_.at(document_id).status, documents_.at(document_id).rating)) {
